...
final class ReservationController extends AbstractController
{
    ...
    public function cancel(
        ...
        if (!$this->isCsrfTokenValid('reservation_cancel_' . $reservation->getId(), $request->request->get('_token'))) {
            $this->addFlash('error', 'Token CSRF invalide.');

            return $this->redirectToRoute('app_reservation_show', ['id' => $reservation->getId()]);
        }

        if ((string) $reservation->getGuest()?->getId() !== (string) $user->getId()) {
        ...
        $bus->dispatch(new BookingCancelledMessage((string) $reservation->getId(), $reason, 'voyageur'));
