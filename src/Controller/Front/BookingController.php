...
final class BookingController extends AbstractController
{
    ...
    public function checkout(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $data = $form->getData();
            $checkin = \DateTimeImmutable::createFromMutable($data['checkinDate']);
            $checkout = \DateTimeImmutable::createFromMutable($data['checkoutDate']);
            $guestsCount = (int) $data['guestsCount'];

            if ($checkin >= $checkout) {
                $this->addFlash('error', 'La date de départ doit être postérieure à la date d\'arrivée.');

                return $this->render('front/property/booking.html.twig', ['property' => $property, 'form' => $form]);
            }
            ...
            $statusMsg = $property->isInstantBooking()
