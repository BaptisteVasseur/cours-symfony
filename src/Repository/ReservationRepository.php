...
class ReservationRepository extends ServiceEntityRepository
{
    ...
    {
        $qb = $this->createQueryBuilder('r')
            ->addSelect('p', 'm', 'a', 'g', 'gp', 'hostUser', 'hostProfile')
            ->addSelect("(CASE WHEN r.status = 'pending' THEN 0 ELSE 1 END) AS HIDDEN statusPriority")
            ->leftJoin('r.property', 'p')
            ->leftJoin('p.media', 'm')
            ->leftJoin('p.address', 'a')
            ->leftJoin('r.guest', 'g')
            ->leftJoin('g.profile', 'gp')
            ->leftJoin('p.host', 'hostUser')
            ->leftJoin('hostUser.profile', 'hostProfile')
     ...
     * @return list<Reservation>
     */
    public function findConfirmedForPropertyAndPeriod(
        Property $property,
